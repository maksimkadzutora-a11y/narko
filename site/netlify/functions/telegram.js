/* ═══════════════════════════════════════════════════════
   Netlify Function — серверная отправка в Telegram
   Токен бота НИКОГДА не попадает в браузер
   ═══════════════════════════════════════════════════════ */

const RATE_MAP = {};
const RATE_LIMIT = 10;       // максимум запросов с одного IP
const RATE_WIN   = 60_000;   // за 60 секунд
...
exports.handler = async (event) => {
  ...
  /* ── Защита по домену (Origin / Referer) ── */
  ...
  /* ── Токен и чат из переменных окружения ── */
  const TGT = process.env.TG_TOKEN;
  const TGC = process.env.TG_CHAT_ID;

  if (!TGT || !TGC) {
    console.error('TG_TOKEN или TG_CHAT_ID не заданы в Environment Variables!');
